...
class Network:
    ...
    def __init__(self, layer_name, m_w_init, v_w_init, a_init, b_init):
        ...
        self.a = tf.get_variable(layer_name + "_a", initializer=float(a_init))
        self.b = tf.get_variable(layer_name + "_b", initializer=float(b_init))
    ...
    def logZ_Z1_Z2_repgression(self, x, y):
        ...
        logZ = -0.5 * (tf.log(v_final) + (y - m)**2 / v_final)
        logZ1 = -0.5 * (tf.log(v_final1) + (y - m)**2 / v_final1)
        logZ2 = -0.5 * (tf.log(v_final2) + (y - m)**2 / v_final2)

        return (logZ, logZ1, logZ2)

    def logZ_Z1_Z2_classification(self, x, y):
        """

        :param x: training input
        :param y: training output class 0 or 1
        :return:
        """
    ...
    def generate_updates(self, logZ, logZ1, logZ2):
        ...
        for i in range(len(self.params_m_w)):
            updates.append((self.params_m_w[ i ], self.params_m_w[ i ] + \
                self.params_v_w[ i ] * tf.grad(logZ, self.params_m_w[ i ])))
            updates.append((self.params_v_w[ i ], self.params_v_w[ i ] - \
               self.params_v_w[ i ]**2 * \
                (tf.grad(logZ, self.params_m_w[ i ])**2 - 2 * \
                tf.grad(logZ, self.params_v_w[ i ]))))

        updates.append((self.a, 1.0 / (tf.exp(logZ2 - 2 * logZ1 + logZ) * \
            (self.a + 1) / self.a - 1.0)))
        updates.append((self.b, 1.0 / (tf.exp(logZ2 - logZ1) * (self.a + 1) / \
            (self.b) - tf.exp(logZ1 - logZ) * self.a / self.b)))
