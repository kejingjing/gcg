...
def run(params):
    ...
    for step in range(int(params['policy']['train_steps'])):
        ...
        if step > 0 and step % int(params['alg']['save_every_n_steps']) == 0:
            itr = step // int(params['alg']['save_every_n_steps']) - 1
            policy.save(train_policy_file_name(itr), train=True)
            policy.save(inference_policy_file_name(itr), train=False)
